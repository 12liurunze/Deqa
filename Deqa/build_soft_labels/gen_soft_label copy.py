...
def parse_args():
    ...
    parser.add_argument("--config", type=str, default="/home/lrz/Deqa/build_soft_labels/config copy.json")
...
def main(cfg):
    ...
    with open(cfg["split_json"]) as fr:
        split = json.load(fr)
    df = pd.read_csv(cfg["mos_csv"])
    mos_dict = df["fidelity"].values
    mos_dict = [round(mos, 3) for mos in mos_dict]  # round to 3 decimal places
    save_train = cfg["save_train"]
    img_dir = cfg["img_dir"]
    img_data = df["filename"].values
    moses, stds, imgs = [], [], []
    for i in range(len(mos_dict)):
        moses.append(mos_dict[i])
        stds.append(0.8)
